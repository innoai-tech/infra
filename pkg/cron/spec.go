...
func (spec Spec) TimeSeq(ctx context.Context) iter.Seq[time.Time] {
	s, _ := cron.ParseStandard(string(spec))
	return TimeSeq(ctx, s)
}
...
func TimeSeq(ctx context.Context, schedule cron.Schedule) iter.Seq[time.Time] {
	if schedule == nil {
